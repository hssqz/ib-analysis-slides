...
```sh
python3 scripts/check_package.py --language en /path/to/output.html
node scripts/capture_pages.cjs /path/to/output.html /path/to/new-qa-directory
```

`--language` checks the declared HTML language against the selected profile. Goldman requires `en`; the shared checker can also check `zh` for future Chinese-language profiles without changing Goldman rules. It does not infer the language of visible prose; that remains part of review. Package examples are always checked as English.
...
View every actual screenshot at its intended size. Compare a relevant package example at the same canvas size: verify role weight, tracking/case, italic qualification, object-to-text color and the actual geometry of selected grouping components. A generic bold heading with an underline cannot stand in for every source role. Keep a brief record of intentional adaptations; do not add visual devices where the content lacks their relationship. Verify title/body/footnote hierarchy, table rows, label association, long labels, signs, zero lines, source visibility, alignment and meaningful use of space. Compare the selected theme and examine the whole deck for repeated evidence or inconsistent scopes. If the preview appears suspicious, verify actual pixels/DOM before changing correct coordinates.
