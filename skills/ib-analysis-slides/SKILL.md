...
description: Turn research and financial evidence into dense, editable investment-bank-style HTML slides. Supports Goldman Sachs-inspired white analytical and FY2024 financial-reporting profiles. Use for industry, company, market, operating-model and financial analysis, reference reconstruction or revisions.
---
...
3. For Goldman Sachs, read [profile routing](banks/goldman/PROFILE.md), then only the relevant theme rules and matching public HTML examples. Currently this is the only supported bank. Other banks, dark themes or unsupported tasks require additional references; changing the palette does not establish a new institutional style.
...
- Build one `article.slide` per page, at 960×540. Use native text and tables plus editable SVG shapes. Scope page CSS and SVG IDs; do not flatten pages into screenshots. Keep output static, with no runtime JavaScript, remote fonts or network resources. HTML tables belong outside SVG; avoid `foreignObject`.
